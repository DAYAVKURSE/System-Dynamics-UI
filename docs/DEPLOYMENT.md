...
| `DEPLOY_PATH` | Абсолютный путь на сервере, например `/opt/system-dynamics-ui` | выбираете сами при выполнении шага 3 |
...
```bash
# 3.1 Node.js 20 (если ещё не стоит)
...
2. Job `deploy` (только если `test` прошёл и ветка — `main`):
   ...
   - пишет `.env` из секрета `TELEGRAM_BOT_TOKEN`;
   - копирует всё на сервер по `rsync --delete` в `${DEPLOY_PATH}/app`
     (каталог с данными `${DEPLOY_PATH}/data` не трогается — он вне `app/`);
   - по SSH ставит прод-зависимости (`npm ci --omit=dev`) и перезапускает
     процесс через `pm2 startOrReload`.
