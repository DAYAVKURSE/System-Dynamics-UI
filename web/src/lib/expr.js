/* ════════════════════════════════════════════════════════════════
   ...
   и ресурсы строки процесса (по порядку) получают буквы «A», «B», «C»…
   — заглавные латинские (владелец, 2026-09-15); буква в операции —
   количество ТОГО ресурса на одном выполнении: «50% A». Строчная «a»
   читается как «A». Кириллическая буква ссылкой не считается — она
   часть имени. У функции буква хранится по
...
/** Буквы ресурсов по порядку: «A», «B», «C»… — заглавные латинские. */
export const LETTERS = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
export const letterOf = (i) => LETTERS[i] || `#${i + 1}`;
/** Номер буквы (регистр не важен); −1, если это не латинская буква. */
export const letterIndex = (ch) => {
  const c = String(ch || "").toUpperCase();
  return c.length === 1 ? LETTERS.indexOf(c) : -1;
};
...
export function parseExpr(text = "") {
  ...
  const factor = () => {
    ...
    if (/[a-zA-Zа-яА-ЯёЁ]/.test(s[i]) && !ALNUM.test(s[i + 1] || "")) {
      ...
      if (k < 0) return fail(`«${s[i]}» — не буква ресурса: буквы латинские, A, B, C…`);
      i += 1;
