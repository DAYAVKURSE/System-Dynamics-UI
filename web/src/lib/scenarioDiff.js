/* ═══════════ РАЗНИЦА ВЕРСИЙ СХЕМЫ ═══════════
   ...
   Изменение сущности — это ЗАМЕНА: старая строка и новая, вставшая на её
   место (владелец, 2026-09-20: «если заменён — в жёлтой»). Прежде пара
   разъезжалась по зелёной и красной плашкам, и то, что одна строка встала
   вместо другой, человек достраивал сам. */
import { diffLines, pairChanges, similar } from "./docdiff.js";
...
/* Вид строки — всё до первого двоеточия: «актив», «ресурс», «функция»,
   «процесс «Приём»». Заменой считаем строки ОДНОГО вида: переименованный
   актив — замена, а актив вместо ресурса — нет, это две разные правки. */
const kindOf = (l) => String(l || "").slice(0, String(l || "").indexOf(":") + 1);
const isPair = (from, to) => !!kindOf(from) && kindOf(from) === kindOf(to) && similar(from, to);

/** Что добавилось, что заменено и что убралось между версиями схемы. */
export function diffDocs(a = {}, b = {}) {
  const { added, removed } = diffLines(docLines(a), docLines(b));
  return pairChanges(removed, added, isPair);
}
