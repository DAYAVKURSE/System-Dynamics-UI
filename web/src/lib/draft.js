...
/* Версия черновика поднята трижды: сперва документ стал другим (вместо
   ...
   прежних записей был убран совсем, и наконец цель уехала с ресурса в
   отдельную часть документа. Черновик прежней версии восстанавливать
   нельзя: он собрал бы модель, которой это приложение уже не понимает. */
export const DRAFT_V = 4;
...
export function readDraft() {
  try {
    ...
    // классификации, функции, задачи и цели. Прежних edges/okrs/hypos в нём
    // нет — расчёта по ним не осталось, и требовать их значило бы не
    // принимать ни одного сегодняшнего черновика.
    const parts = ["entities", "traits", "kinds", "funcs", "tasks", "goals"];
