...
export const newFunc = (e, name = "новая функция") => ({
  ...
  /* Сколько выполнений идут ОДНОВРЕМЕННО, и это два разных вопроса.

     `par` — сколько их держит ОДИН ВОРКЕР. По умолчанию одно: работа
     ...
     `parAll` — сколько их идёт В АКТИВЕ ВСЕГО. Ноль — «не ограничено»:
     молчание не должно означать «одно», иначе всякая прежняя схема с
...
/**
 * Сколько выполнений этой функции держит ОДИН ВОРКЕР одновременно.
 *
 ...
 */
export const parWorkerOf = (f = {}) => {
  const n = Math.floor(num(f.par));
  return n > 1 ? n : 1;
};

/**
 * Сколько выполнений идёт В АКТИВЕ ВСЕГО. Ноль — «не ограничено».
 *
 * Потолок всей работы, а не одного человека: станок один, кабинет один,
 * лицензий три — сколько бы людей ни было, разом идёт столько. Пусто
 * читается как «предела нет», а не как «одно»: молчание не запрет.
 */
export const parAssetOf = (f = {}) => {
 ...
 * считаем (инвариант 6 — число исполнителей на скорость не влияет), а
 * значит и умножать «на воркера» на их количество нельзя. Потолок актива
 * при этом обязан работать: сказано «больше трёх разом не идёт» — значит
 * не идёт, даже если один человек взялся бы за восемь.
 */
export const parOf = (f = {}) => {
  const one = parWorkerOf(f);
  const all = parAssetOf(f);
  return all > 0 ? Math.min(one, all) : one;
};
...
export const normalizeFunc = (f = {}) => {
  ...
  const port = (p = {}, grouped = false) => {
    ...
    return {
      ...
      // Операция количества («50% а», «20% @{t1}») — остаётся, если была.
      ...(p.expr ? { expr: String(p.expr) } : {}),
  ...
  return {
    ...
    /* Меньше одного одновременного выполнения на воркера не бывает: ноль
       означал бы, что работа не делается вовсе, а этого поле не про то.
       Дробей тут тоже нет — половины дела в работе не держат. У потолка
...
/**
 * Порты — по одному на ресурс.
 *
 * Ресурс с несколькими получателями («Кому:» дважды) — это несколько портов
 * (расчёту нужен каждый: у каждого свой адрес), но ОДИН ресурс. Там, где
 * человека спрашивают или ему говорят про ресурс — плашка функции, «сколько
 * выдали», «вещи, которые вышли», — он должен встречаться один раз
 * (владелец, 2026-09-20: «два раза написано „возможный диапазон времени";
 * должно быть написано только один раз»). Остаётся первый порт: вилка у
 * них общая, она из одной строки «Отдаёт:».
 */
export const uniqPorts = (ports = []) => {
  const seen = new Set();
  return ports.filter((p) => {
    const k = p?.trait ?? p?.id;
...
/**
 * Чего в сдаче не хватает: обязательный выход, где приложено не всё.
 *
 * Сколько единиц сдают — столько и прикладывают: сдал десять договоров —
 * десять файлов. `got` — сколько единиц уже заполнено по ресурсу, `need`
 * — сколько их сдают. Не названо сколько — считаем, что одну: так вела
 * себя прежняя сдача, и прежние вызовы не должны менять смысл.
 *
 ...
 */
export const missingGives = (f, got = {}, need = {}) =>
  requiredGives(f).filter((p) => {
    const want = Math.max(1, Math.floor(num((need || {})[p.trait])) || 1);
    const have = (got || {})[p.trait];
    // Прежний вызов передавал сам файл, а не число: он значит «есть один».
    const n = typeof have === "number" ? have : (have ? 1 : 0);
    return n < want;
  });
