...
export const normalizeReport = (n = {}) => {
  ...
  return {
    ...
    /* Ресурсы, которые решено НЕ прослеживать (галочки в разделе
       «Ресурсы», владелец 2026-09-19): в отчёт они не идут. Часть записи —
       иначе выбор терялся бы при первом же сохранении. */
    off: [...new Set((Array.isArray(n.off) ? n.off : []).map(str).filter(Boolean))],
  };
};
