...
/* ─── Цвет с прозрачностью (владелец, 2026-09-22) ───
   Токены — переменные CSS, и приклеенный к ним hex-хвост («${ACC}66»)
   давал негодный цвет: браузер молча выбрасывал правило — так пропали
   линии линейки на таймлайне процесса и ещё десяток рамок и подложек.
   `a` — те же две hex-цифры (00–ff), что писали раньше. */
export const alpha = (color, a) => {
  const c = String(color || "");
  if (/^#[0-9a-f]{6}$/i.test(c)) return `${c}${a}`;
  const pct = Math.round((parseInt(a, 16) / 255) * 100);
