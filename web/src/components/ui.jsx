...
/* ─────── ЗНАК И ИМЯ ПРИЛОЖЕНИЯ ───────
   ...
   Владелец (2026-09-19) прислал знак и сказал поставить его слева от
   названия: куб в изометрии, а внутри — дерево связей с точками на
   концах. Знак РИСУЕТСЯ, а не кладётся картинкой: он живёт в своём цвете,
   его нечего грузить, он чёток на любом экране и не тащит за собой чёрный
   фон исходника.

   Грани куба — тот же рисунок, что и связи внутри: ствол идёт от нижнего
   угла вверх по вертикальному ребру, а ветви расходятся по трём граням.
   Поэтому ребро и ствол — одна линия, а не две наложенные. */
export const BRAND = "#2FDD6A";
/* Куб: шестиугольник со скруглёнными углами (посчитан по вершинам, радиус 5). */
const CUBE = "M19.7,6.5 Q24.0,4.0 28.3,6.5 L37.2,11.5 Q41.5,14.0 41.5,19.0 L41.5,29.0 "
  + "Q41.5,34.0 37.2,36.5 L28.3,41.5 Q24.0,44.0 19.7,41.5 L10.8,36.5 Q6.5,34.0 6.5,29.0 "
  + "L6.5,19.0 Q6.5,14.0 10.8,11.5 Z";
/* Ветви: по паре на каждую грань и три на верхнюю — как на присланном знаке. */
const TWIGS = [
  "M24,21.5 Q20.5,18 17.6,14.8", "M24,21.5 Q27.5,18 30.4,14.8",
  "M24,29.5 Q18,27.5 11.4,25.2", "M24,29.5 Q30,27.5 36.6,25.2",
  "M24,35.5 Q20.5,34 16.8,31.6", "M24,35.5 Q27.5,34 31.2,31.6",
];
const BUDS = [[24, 11.6], [17.6, 14.8], [30.4, 14.8], [11.4, 25.2], [36.6, 25.2],
  [16.8, 31.6], [31.2, 31.6]];
export function Brand({ size = 26, color = BRAND }) {
  return (
    <span className="flex items-center gap-2" aria-label="Blocktree">
      <svg width={size} height={size} viewBox="0 0 48 48" aria-hidden="true"
        style={{ display: "block", flex: "0 0 auto",
          filter: `drop-shadow(0 0 ${Math.max(1, size / 14)}px ${color}80)` }}>
        <g fill="none" stroke={color} strokeWidth="1.8" strokeLinecap="round"
          strokeLinejoin="round">
          <path d={CUBE} />
          {/* Рёбра, сходящиеся в середине: две верхние грани и ствол вниз. */}
          <path d="M24,24 L8.6,15.2" strokeWidth="1.5" />
          <path d="M24,24 L39.4,15.2" strokeWidth="1.5" />
          {/* Ствол — от нижнего угла вверх сквозь середину. */}
          <path d="M24,42.6 L24,12.6" />
          {TWIGS.map((d) => <path key={d} d={d} strokeWidth="1.4" />)}
        </g>
        {BUDS.map(([x, y]) => <circle key={`${x}:${y}`} cx={x} cy={y} r="2" fill={color} />)}
      </svg>
