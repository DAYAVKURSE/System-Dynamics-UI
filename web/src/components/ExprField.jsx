        ...
        <div className="flex flex-wrap gap-2" style={{ marginTop: "var(--space-4)" }}>
          {ports.map((p) => (
            <button key={p.id} aria-label={`буква ${p.letter} — ${p.name}`} type="button"
              ...
              style={{ ...btn(false), paddingTop: "calc(var(--btn-py) + var(--text-nudge))", paddingBottom: "calc(var(--btn-py) - var(--text-nudge))", paddingLeft: "var(--space-8)", paddingRight: "var(--space-8)", minWidth: 26,
          ...
          {(plain ? PLAIN_KEYS : KEYS).map((k) => (
            <button key={k} aria-label={`знак ${k}`} type="button"
              ...
              style={{ ...btn(false), fontFamily: "var(--font-sans)", paddingTop: "calc(var(--btn-py) + var(--text-nudge))", paddingBottom: "calc(var(--btn-py) - var(--text-nudge))", paddingLeft: "var(--space-8)", paddingRight: "var(--space-8)", minWidth: 26 }}>{k}</button>))}
        </div>)}
      {pick && !!items.length && (
        <div role="listbox" aria-label="ресурсы для выражения"
          ...
          {items.map((t, i) => (
            <div key={t.id} role="option" aria-selected={i === cursor}
              ...
              style={{ padding: "var(--space-4) var(--space-8)", fontSize: "var(--fs-hint)", cursor: "pointer",
                background: i === cursor ? ACC + "22" : "transparent" }}>
              @{t.l}</div>))}
        </div>)}
      {err && text.trim() && (
        <div style={{ fontSize: "var(--fs-hint)", color: BAD, marginTop: "var(--space-4)" }}>{err}</div>)}
    </div>);
}
