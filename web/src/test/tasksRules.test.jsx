  ...
  it("порядок полей: название, функция, описание, критерии, сроки (владелец, 2026-09-19)", () => {
    ...
    const labels = [...container.querySelectorAll("div")]
      ...
      .filter((x) => ["название", "выполняемая функция", "описание задачи",
        "критерии проверки", "начать"].includes(x));
    ...
    /* Комментариев в задаче больше нет вовсе (владелец, 2026-09-20): их
       заменило обсуждение, и открывается оно с плашки задачи. */
    expect(screen.queryByText("что сказали в задаче")).toBeNull();
...
describe("критерии на форме постановки", () => {
  ...
  it("плашка функции — берёт, ожидается, срок и ожидаемый результат, каждое с подписью", () => {
    ...
    /* Описания функции на плашке НЕТ (владелец, 2026-09-20: «убери описание
       функции с плашки функции; описание должно быть только у задачи»). */
    expect(plate.textContent).not.toMatch(/описание/i);
    // Безымянного числа «заложено» больше нет.
    expect(plate.textContent).not.toMatch(/на одно выполнение заложено/);
    // Порядок: срок → ожидаемый результат, он же последний.
    const at = (w) => plate.textContent.indexOf(w);
    expect(at("Срок:")).toBeLessThan(at("Ожидаемый результат:"));
    // Списка критериев в плашке по-прежнему нет: он своим разделом на форме.
    expect(plate.textContent).not.toMatch(/критерии проверки/);
  });

  it("результат не назван — так и написано, и сказано где его ставят", () => {
...
describe("поля задачи в работе на «Проверке»", () => {
  const F = [{ ...FUNCS[0], checks: ["есть запись"] }];
  ...
  const show = (t, funcs = F) => render(<Review tasks={[t]} funcs={funcs} />);
  ...
  it("каждое поле — своей строкой, с названием и двоеточием", () => {
    ...
    // Описание — задачи, а не функции (владелец, 2026-09-20).
    expect(rows[3]).toBe("описание: позвонить первым делом");
    expect(rows[4]).toMatch(/^критерии проверки: /);
    expect(box.textContent).not.toMatch(/содержимое/);
  ...
  it("пустое поле названо словами, а не пропущено", () => {
    show(работа({ body: "" }), [{ ...FUNCS[0], checks: [] }]);
    fireEvent.click(screen.getByText("Задача"));
