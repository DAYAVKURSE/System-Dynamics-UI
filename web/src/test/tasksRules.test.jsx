...
describe("критерии на форме постановки", () => {
  ...
  it("плашка функции — берёт, выдаёт, срок и ожидаемый результат, каждое с подписью", () => {
    ...
    /* Описание функции — после ожидаемого результата, со своей подписью
       (владелец, 2026-09-20: «должно быть также видно на плашке функции…
       после ожидаемого результата; так и должно быть написано: „описание:"»). */
    expect(plate.textContent).toMatch(/описание: описание функции/);
    // Безымянного числа «заложено» больше нет.
    expect(plate.textContent).not.toMatch(/на одно выполнение заложено/);
    // Порядок: срок → ожидаемый результат → описание.
    const at = (w) => plate.textContent.indexOf(w);
    expect(at("Срок:")).toBeLessThan(at("Ожидаемый результат:"));
    expect(at("Ожидаемый результат:")).toBeLessThan(at("описание:"));
  ...
  it("результат не назван — так и написано, и сказано где его ставят", () => {
    ...
    expect(plate.textContent).toMatch(/описание: не написано — его пишут у функции на «Схеме»/);
  });
