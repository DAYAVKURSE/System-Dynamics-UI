...
/* ВОРКЕРЫ АКТИВА ОДНИМ СПИСКОМ.
   ...
   Роли отвечают на вопрос «кто чем занят», а список воркеров — на другой:
   «кто вообще работает в этом активе». Членство задаётся прямо, порядок в
   нём задаёт человек, и в этом же порядке людей предлагают потом в формах
   выбора. */
describe("порядок людей актива", () => {
  ...
  it("воркер задаётся прямо, а не тем, что его куда-то назначили", () => {
    /* Сперва отмечают воркеров актива, и уже из них выбирают постановщиков,
       исполнителей и проверяющих. Поэтому в списке есть и тот, кому роли
       ещё не досталось. */
    expect(crewOf({ ...W, crew: ["сам", "a"] })).toEqual(["сам", "a", "b", "c"]);
    // Назначенный в роль считается воркером и без отметки: так открываются
    // модели, собранные прежним порядком.
    expect(crewOf(W)).toEqual(["b", "a", "c"]);
  });
