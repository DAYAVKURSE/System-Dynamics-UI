...
/* ОШИБКИ СИНТАКСИСА СЛОВАМИ (владелец, 2026-09-22): что введено не так,
   написано под полем; повтор «Кто:»/«Берёт:»/«Отдаёт:» в одной задаче —
   ошибка: одна задача — одно действие. */
describe("ошибки синтаксиса словами", () => {
  const model = { entities: [{ id: "a", name: "Партнёр", posts: ["r1"] }], positions: [{ id: "r1", name: "Сборщик" }],
    traits: [{ id: "t1", l: "оплата", e: "a" }] };
  const errs = (text) => parseText(text, model, {}).errors.map((e) => `${e.line}: ${e.message}`);

  it("повтор «Кто:» и «Отдаёт:» в одной задаче — ошибка с номерами строк", () => {
    const out = errs("Задача: передать\nКто: Партнёр\nОтдаёт: оплата\nКто: Сборщик\nОтдаёт: оплата 2");
    expect(out).toContainEqual(expect.stringMatching(/4: в задаче «передать» «Кто:» повторяется \(строки 2, 4\) — одна задача — одно действие: ещё одного участника пишите следующей строкой без метки/));
    expect(out).toContainEqual(expect.stringMatching(/5: в задаче «передать» «Отдаёт:» повторяется \(строки 3, 5\)/));
    // Ветки условия — разные действия, повтором не считаются.
    expect(errs("Задача: x\nЕсли: a\nКто: Партнёр\nОтдаёт: оплата\nИначе:\nКто: Сборщик\nОтдаёт: оплата")).toEqual([]);
    expect(issuesOf({ text: "Задача: x\nКто: Партнёр\nОтдаёт: оплата\nКто: Партнёр\nОтдаёт: оплата" }, model).join(" | ")).toMatch(/строка 4: в задаче «x» «Кто:» повторяется/);
  });

  it("метка без двоеточия или с другим знаком, метка с опечаткой", () => {
    expect(errs("Задача: x\nКто Партнёр\nОтдаёт: оплата")).toContainEqual("2: «Кто» — после метки нужно двоеточие: «Кто: …»");
    expect(errs("Задача: x\nКто; Партнёр\nОтдаёт: оплата")).toContainEqual("2: «Кто;» — после метки должно стоять двоеточие, а не «;»: «Кто: …»");
    expect(errs("Задача: x\nКто: Партнёр\nОтдаёт. оплата")).toContainEqual("3: «Отдаёт.» — после метки должно стоять двоеточие, а не «.»: «Отдаёт: …»");
    expect(errs("Задача: x\nКто: Партнёр\nОтдоёт: оплата")).toContainEqual("3: «Отдоёт:» — такой метки нет; похоже на «Отдаёт:»");
    expect(errs("Задача: x\nКто: Партнёр\nОт каго: оплата")).toContainEqual(expect.stringMatching(/3: «От каго:» — такой метки нет; похоже на «От кого:»/));
  });

  it("пустая метка, скобки, «Кому:» после «Берёт:», «Если:» без «То:»", () => {
    expect(errs("Задача: x\nКто: Партнёр\nОтдаёт:")).toContainEqual("3: «Отдаёт:» — назовите ресурс");
    expect(errs("Задача: x\nКто: Партнёр\nОтдаёт: оплата =1 (saddle")).toContainEqual("3: не закрыта скобка «(»");
    expect(errs("Задача: x\nКто: Партнёр {kind sparrow\nОтдаёт: оплата")).toContainEqual("2: не закрыта скобка «{»");
    expect(errs("Задача: x\nКто: Партнёр\nОтдаёт: оплата)")).toContainEqual("3: лишняя закрывающая скобка «)»");
    expect(errs("Задача: x\nКто: Партнёр\nБерёт: оплата\nКому: Сборщик")).toContainEqual("4: «Кому:» после «Берёт:» — кто отдаёт, пишется в «От кого:»");
    expect(errs("Задача: x\nКто: Партнёр\nОтдаёт: оплата\nОт кого: Сборщик")).toContainEqual("4: «От кого:» после «Отдаёт:» — кто получает, пишется в «Кому:»");
    expect(errs("Задача: x\nКто: Партнёр\nОтдаёт: оплата\nКому:")).toContainEqual("4: «Кому:» — назовите, кому");
    expect(errs("Задача: x\nКто: Партнёр\nОтдаёт: оплата\n\nЕсли: a > 0")).toContainEqual("5: «Если:» — после условия нужны «То:» и задача");
  });
});
