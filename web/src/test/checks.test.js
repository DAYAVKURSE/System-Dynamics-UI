...
describe("функция", () => {
  ...
  it("объяснение — то, что дал владелец, нынешними словами", () => {
    /* Определение одной строкой, под ним — список условий. Прежде здесь
       лежал абзац из семи предложений: его не дочитывали, и главное — из
       него не было видно, что именно надо сделать. */
    expect(checkFunc(F("f1", "A", [], []), m).why).toBe(WHY_FUNC);
    expect(WHY_FUNC).toMatch(/^Функция обменивает одни ресурсы на другие\./);
    expect(WHY_FUNC).toMatch(/Чтобы быть частью актива/);
    // Каждое условие списка — то, что и правда проверяется.
    expect(WHY_FUNC).toMatch(/что берёт/);
    expect(WHY_FUNC).toMatch(/что выдаёт/);
    expect(WHY_FUNC).toMatch(/за какое время выполняется/);
  });
});
