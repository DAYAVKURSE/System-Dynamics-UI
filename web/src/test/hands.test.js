...
import { applyHand, handLinks, handOfFunc, linkPath, pinsOf, removeHand } from "../lib/hands.js";
...
describe("пины и линии", () => {
  it("по пину на задачу процесса справа от блока; рука — у исполнителя", () => {
    ...
    expect(pins.map((p) => [p.func, p.x, p.y, p.hand])).toEqual([["pr1_1_t1", 318, 68, "A"], ["pr1_1_t2", 318, 83, "A"], ["pr1_1_t3", 318, 98, null]]);
    expect(handOfFunc(funcs[2])).toBeNull();
  });
  it("задачи с одной рукой соединены по порядку шагов; путь идёт вправо со скруглениями", () => {
    expect(handLinks(funcs)).toEqual([{ a: "pr1_1_t1", b: "pr1_1_t2", hand: "A", proc: "pr1" }]);
