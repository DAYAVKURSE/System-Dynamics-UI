...
describe("что видно по ссылке", () => {
  ...
  it("вещь не выбрана — наружу уходит прогноз, а не чужая работа", async () => {
    /* Раздел без выбранных единиц отвечает на вопрос «что произойдёт, когда
       вещь появится». Показывать вместо этого выполнения тех же функций над
       ЧУЖИМИ вещами нельзя: отслеживая один контакт лида, заказчик видел бы
       четыре одинаковых «передать заказ разработчикам» и справедливо не
       понимал, при чём тут его. Правило то же, что в приложении. */
    const { body } = await share("rs1");
    const { body: got } = await request(app).get(`/api/shares/${body.token}`);
    const b = got.snapshot.block;
    ...
    expect(b.plan.steps).toHaveLength(1);
    // У раздела отчёта свой якорь: ссылка снаружи ведёт туда же, что внутри.
    expect(b.plan.steps[0].anchor).toBe("shag-rs1-f1");
    /* И свой прогноз ресурсов — тот же набор данных, что у отчёта целиком:
       шаг берёт заявку и выдаёт макет. */
    const byTrait = Object.fromEntries(
      b.plan.steps[0].changes.map((c) => [c.trait, c]));
    expect(byTrait["заявка"].hi).toBe(-1);
    expect(byTrait["макет"].hi).toBe(1);
  });
