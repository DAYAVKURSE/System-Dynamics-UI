...
import { languages, langOf, resetDicts, setUserLang, tr, trKeyboard, userLang } from "../lib/i18n.js";
...
beforeAll(async () => {
  ...
  await fs.writeFile(path.join(process.env.LOCALES_DIR, "English.json"), JSON.stringify({
...
describe("перевод исходящего", () => {
  it("русский — как есть; английский — по словарю, построчно и по шаблону; чего нет — по-русски", () => {
    expect(languages()).toEqual(["Русский", "English"]);   // файл — язык, русский первым
    expect(langOf("xx")).toBe("Русский");
    expect(langOf("en")).toBe("English");   // старый код
    expect(tr("Русский", "Задача принята.")).toBe("Задача принята.");
    expect(tr("English", "Задача принята.")).toBe("Task accepted.");
    expect(tr("English", "Осталось 3 дн. до «Сайт»")).toBe("3 days left until “Сайт”");
    expect(tr("English", "Задача принята.\nТакого нет\nВзять")).toBe("Task accepted.\nТакого нет\nTake");
    expect(tr("English", "no cyrillic")).toBe("no cyrillic");
    // Собранное из кусков — по кускам.
    expect(tr("English", "Осталось 7 дн. до сдачи")).toBe("Left 7 d. until submission");
  });
  it("клавиатура: подписи переводятся, данные кнопок — нет", () => {
    ...
    expect(trKeyboard("English", kb)).toEqual({ inline_keyboard: [[{ text: "Take", callback_data: "take:1" }, { text: "Открыть", url: "https://x" }]] });
    expect(trKeyboard("Русский", kb)).toBe(kb);
    expect(trKeyboard("English", null)).toBeNull();
  });
  it("язык человека хранится в langs.json; русский — значит, записи нет", async () => {
    expect(userLang("100")).toBe("Русский");
    await setUserLang("100", "en");   // старый код принимается
    expect(userLang("100")).toBe("English");
    expect(JSON.parse(await fs.readFile(path.join(process.env.ORG_DIR, "langs.json"), "utf8"))).toEqual({ 100: "English" });
    await setUserLang("100", "Русский");
    expect(userLang("100")).toBe("Русский");
  });
  ...
  it("язык кладётся в анкету и оттуда — в langs.json", async () => {
    ...
    await fs.writeFile(path.join(process.env.LOCALES_DIR, "中文.json"), "{}");
    resetDicts();
    const saved = await org.setProfile("100", { lang: "zh" });   // старый код → файл
    expect(saved.lang).toBe("中文");
    expect(userLang("100")).toBe("中文");
    // Незнакомый язык — русский.
    expect((await org.setProfile("100", { lang: "nope" })).lang).toBe("Русский");
    expect(userLang("100")).toBe("Русский");
  });
});
