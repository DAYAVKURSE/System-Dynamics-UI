...
/* ════════════════════════════════════════════════════════════════
   ...
   бота, секрет моста и секрет TURN — потерять их значило бы сломать всё
   сразу. Права 0600: в этом файле секреты.
...
export function putEnvValue(name, value, file = envFile()) {
  ...
  // файл, а воркер читает его на каждом круге опроса — и мог бы прочитать
  // половину токена и принять её за новый вход. Переименование атомарно:
