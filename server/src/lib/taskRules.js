...
/**
 * «Не менять руку» (зеркало `handMate` из `web/src/lib/funcs.js`): кто уже
 * назначен на роль в другой задаче того же процесса с той же рукой.
 */
export function handMate(model = {}, task = {}, role) {
  const f = (model.funcs || []).find((x) => x.id === task.funcId) || null;
