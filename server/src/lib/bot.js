...
/* Карточка приглашения.
   ...
   Ссылок в ней две, и вторая не для красоты. Первая открывает окно
   звонка мини-приложением — так и задумано, там надёжно работают камера и
   ...
   которому человеку нечего понять. Вторая ведёт на ту же страницу напрямую
   и не зависит ни от чего: по ней войдёт кто угодно, хоть вовсе без
   Telegram. Пусть лучше будет запасной выход, чем «у меня не открывается»
   посреди назначенной встречи.

   Но в ТЕКСТЕ сообщения запасной ссылки больше нет, и это не мелочь.
   Обычная https-ссылка открывается встроенным браузером Telegram, а он
   всегда во весь экран: понятия высоты окна у страницы нет вовсе. Лёжа
   соседней строкой, она на глаз не отличалась от нужной — и «звонок
   открывается на весь экран» объясняется одним промахом пальца. Теперь
   запасной выход — подписанная кнопка под сообщением, мимо которой не
   промахнёшься; превью выключено по той же причине: оно было третьей
   крупной целью, ведущей туда же. */
const meetingCard = (m, link) => [
  `📹 ${m.title}`,
  m.at ? `когда: ${m.at}` : "когда: договоримся в чате",
  "",
  `Подключиться: ${link}`,
  // Про пол-экрана обещаем, только когда это правда, и только там, где она
  // возможна: на компьютере половины не бывает вовсе, а на телефоне её
  // умеет одно лишь главное приложение бота (почему — в lib/links.js).
  // Обещать половину и открыть целое — хуже, чем не обещать.
  isMainAppLink(link)
    ? "\nНа телефоне окно откроется на пол-экрана — потяните вверх, чтобы развернуть."
    : "",
].filter(Boolean).join("\n");

/** Кнопки под приглашением: вход и, отдельно, запасной выход страницей. */
const meetingButtons = (link, page) => [
  [{ text: "📹 Подключиться", url: link }],
  ...(page && page !== link
    ? [[{ text: "Не открылось? Открыть страницей", url: page }]]
    : []),
];
