...
const meetingCard = (m, link, page) => [
  ...
  // Про пол-экрана обещаем, только когда это правда: на весь экран
  // открывается отдельное приложение звонка, и поделать с этим ничего
  // нельзя (почему — в lib/links.js). Обещать половину и открыть целое —
  // хуже, чем не обещать.
  isMainAppLink(link)
    ? "\nЗвонок откроется отдельным окном на пол-экрана — потяните вверх, чтобы развернуть."
    : isAppLink(link)
      ? "\nЗвонок откроется отдельным окном Telegram."
      : "",
].filter(Boolean).join("\n");
...
const HELP = [
  ...
  "",
  "«/callmain» — если звонок открывается на весь экран, а хочется на",
  "половину: расскажу, как сделать его главным приложением бота. Модель",
...
async function onCallMain(arg, from, deps) {
  ...
  if (want !== "on") {
    await send(from.id, on
      ? [`Звонок открывает главное приложение бота: t.me/${botName}?startapp=call_…`,
        "",
        "Так окно открывается на пол-экрана. Если оно всё равно на весь —",
        "проверьте в @BotFather (Bot Settings → Configure Mini App), что там",
        "выбрана половина экрана: на телефоне высоту решает именно эта",
        "настройка, а не ссылка.",
        "",
        "Вернуть отдельное приложение — «/callmain off»."].join("\n")
      : ["Сейчас звонок открывается на весь экран, и по-другому отдельное",
        "приложение не умеет: на десктопе Telegram для таких ссылок половину",
        "не делает вовсе, на телефоне высоту диктует ответ сервера.",
        "",
        "Половина получится, если то же окно звонка сделать ГЛАВНЫМ",
        "приложением бота. Это минута:",
  ...
  await send(from.id, [
    ...
    "окно должно открыться на пол-экрана.",
    "",
    "Если всё равно на весь экран — на телефоне высоту решает не ссылка, а",
    "настройка приложения: @BotFather → Bot Settings → Configure Mini App,",
    "выберите половину экрана. И закройте окно звонка полностью перед",
    "проверкой: свёрнутое Telegram открывает в той же высоте, что и было.",
    "",
