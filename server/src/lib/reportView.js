...
function estimate(model, chain, side, qty) {
  ...
  order.forEach((f) => {
    ...
    /* Выполнения идут волнами по `par` штук, а не строго друг за другом:
       ...
       Пределов два: сколько дел держит ОДИН ВОРКЕР (`par`) и сколько их
       идёт В АКТИВЕ всего (`parAll`, ноль — «не ограничено»). Берётся
...
/* ─────── номера единиц ───────
   ...
   Единица ресурса рождается сдачей задачи или кладётся в «Материалы»
   руками, и номер у неё — порядковый внутри своего ресурса, от старых к
   новым, общий на оба источника. Считается тем же правилом, что и в
   приложении (`web/src/lib/units.js`): снимок обязан называть вещь тем
...
function unitRows(model = {}) {
  const rows = [];
  (Array.isArray(model.materials) ? model.materials : []).forEach((m) => {
    if (!m || !m.trait) return;
    rows.push({ id: str(m.id), trait: m.trait, at: str(m.at), task: null, took: [] });
  });
